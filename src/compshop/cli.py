...
def build_arg_parser():
    ...
    parser.add_argument("--property", required=True, help="MGM Property name (e.g., 'MGM Detroit')")
    parser.add_argument("--competitor", default=None, help="Competitor name override (auto-detected from PDF if omitted)")
    parser.add_argument("--input", required=True, help="Path to folder containing PDF files")
    parser.add_argument("--template", required=True, help="Path to CompShopAgentTemplate.xlsx")
    parser.add_argument("--output", default=".", help="Output directory (default: current dir)")
    parser.add_argument("--model", choices=list(MODELS.keys()), default=DEFAULT_MODEL, help=f"Model to use (default: {DEFAULT_MODEL})")
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE, help=f"PDFs per API call (default: {DEFAULT_BATCH_SIZE})")
    parser.add_argument("--property-keyword", default=None, help="Keyword to match in filenames (default: derived from --competitor or --property)")
    parser.add_argument("--dry-run", action="store_true", help="Scan and qualify PDFs without making API calls")
    parser.add_argument("--latest-only", action="store_true", help="Process only the newest qualifying PDF")
    return parser
...
def main():
    ...
    console.print(f"  [green]✓[/green] Template loaded — {len(ref_values['properties'])} properties, "
    ...
    with console.status(f"[bold blue]Scanning for PDFs matching '{keyword}'..."):
        try:
            qualifying = scan_and_qualify(args.input, keyword, latest_only=args.latest_only)
        except FileNotFoundError as e:
            console.print(f"[red]{e}[/red]")
            sys.exit(1)

    if not qualifying:
        console.print(f"  [yellow]⚠[/yellow]  No qualifying PDFs found for '{keyword}' in {args.input}")
        console.print("  [dim]Tip: use --property-keyword to override filename matching[/dim]")
        sys.exit(0)
    ...
    with console.status("[bold blue]Extracting text from PDFs..."):
        documents = extract_text(qualifying)

    total_chars = sum(d.total_chars for d in documents)
    total_pages = sum(len(d.pages) for d in documents)
    console.print(f"  [green]✓[/green] Extracted {total_pages} pages, {total_chars:,} chars from {len(documents)} PDFs")
    ...
    for doc in documents:
        pdf_table.add_row(doc.filename, doc.segment_code, str(len(doc.pages)), f"{doc.total_chars:,}")
    ...
    api_key = get_api_key()
    if not api_key:
        console.print("[red]No API key found. Set ANTHROPIC_API_KEY env var or create a .env file.[/red]")
        sys.exit(1)
    ...
    if not competitor:
        ...
        for c in ref_values["competitors"]:
            if keyword_lower in c.lower().replace(" ", "").replace(".", ""):
                ...
                break
        if not competitor:
            console.print(f"[yellow]Could not auto-detect competitor for '{keyword}'.[/yellow]")
    ...
    with Progress(
        ...
        result = classify_batches(batches, model_id, system_prompt, api_key, progress_callback=on_batch)

    # Stats
    total_cost = estimate_cost(model_id, result.total_input_tokens, result.total_output_tokens)
    console.print(f"  [green]✓[/green] {len(result.all_offers)} raw offers extracted in {result.total_time:.1f}s")
    console.print(f"  [dim]Tokens:[/dim] {result.total_input_tokens:,} in / {result.total_output_tokens:,} out")
    ...
    console.print(f"  [green]✓[/green] {len(val.valid_offers)} offers after dedup ({val.dupes_removed} duplicates removed)")
    ...
    console.print(f"  [green]✓[/green] Wrote {row_count} rows to [bold]{output_path}[/bold]")
    console.print(f"  [dim]Rows {first_row}–{last_row} | Verified: {verification['occupied_rows']} occupied[/dim]")

    if not verification["count_match"]:
        console.print(f"  [red]⚠ Row count mismatch: expected {row_count}, found {verification['occupied_rows']}[/red]")
    if not verification["placement_ok"]:
        console.print(f"  [red]⚠ Placement issue: first occupied row is {verification['first_occupied_row']}, expected {first_row}[/red]")
    ...
    summary_table = Table(box=box.ROUNDED, show_header=False, border_style="blue", padding=(0, 1))
    ...
    summary_table.add_row("By type", ", ".join(f"{k}: {v}" for k, v in type_counts.most_common()))
    summary_table.add_row("By category", ", ".join(f"{k}: {v}" for k, v in cat_counts.most_common()))
    ...
    console.print(Panel(summary_table, title="[bold]Run Summary[/bold]", border_style="blue"))

    # Preview last 2 rows
    if verification["preview"]:
        preview_table = Table(box=box.SIMPLE, show_header=True, header_style="dim")
        for col in ["Row", "Property", "Competitor", "StartDate", "Type", "Category", "OfferAmt", "Name"]:
